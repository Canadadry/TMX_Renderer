...
ImplementedLoader::ImplementedLoader(std::string filename)
: m_map(NULL)
{
	if(filename != "")
	{
...
void ImplementedLoader::LoadFromFile(std::string filename)
{
...
void ImplementedLoader::readMap(TiXmlNode* node)
{
	PRINT("Reading Map\n");
...
void ImplementedLoader::readProperties(TiXmlNode* node, std::vector<TMXProperty*>& properties)
{
...
TMXTileSet* ImplementedLoader::readTileSet(TiXmlNode* node)
{
...
const char* ImplementedLoader::readImage(TiXmlNode* node)
{
	PRINT("Reading Image\n");
	TiXmlElement* pElement = node->ToElement(); //on le converti en element
...
TMXLayer* ImplementedLoader::readLayer(TiXmlNode* node)
{
	PRINT("Reading Layer\n");
...
void ImplementedLoader::readData(TiXmlNode* node, std::vector<int>& data)
{
	PRINT("Reading Data:");
	TiXmlElement* pElement = node->ToElement(); //on le converti en element
...
void ImplementedLoader::readXML(TiXmlNode* node,std::vector<int>& data)
{
	PRINT("XML format\n");
...
void ImplementedLoader::readCSV(TiXmlNode* node,std::vector<int>& data)
{
	PRINT("CSV format\n");
...
void ImplementedLoader::decodeblock( unsigned char* in, unsigned char* out ) //in must contain 4 char, out must contain 3 char
{
...
void ImplementedLoader::readBase64(TiXmlNode* node,const char* compression,std::vector<int>& data)
{
